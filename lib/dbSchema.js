...
export const MijlpaalFactory = {
  ...
  async createByAdmin({ jaar, titel, beschrijving, file }) {
  ...
  async createPublic({ jaar, titel, beschrijving, contactEmail }) {
    const docRef = await addDoc(collection(db, MIJLPALEN), {
      jaar,
      titel: titel || '',
      beschrijving: beschrijving || '',
  ...
  async update(id, { jaar, titel, beschrijving, file, bestaandePath }) {
    const data = { jaar, titel, beschrijving };
    if (file) {
      if (bestaandePath) await verwijderAfbeelding(bestaandePath);
