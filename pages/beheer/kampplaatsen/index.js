...
function LocatiesContent() {
  ...
  const isGekoppeld = (groep) => {
    const loc = locaties[groep.label.trim().toLowerCase()];
    return Boolean(loc) && !loc.genegeerd;
  };
  const isGenegeerd = (groep) => Boolean(locaties[groep.label.trim().toLowerCase()]?.genegeerd);
    ...
    <div style={{ minHeight: '100vh', background: 'transparent' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <p style={{ fontFamily: fonts.body, fontSize: 14, color: colors.inkMuted, marginBottom: 20 }}>
          ...
          coördinaten verschijnen gewoon niet op de kaart. Is een naam geen
          echte plaats (bv. "Allemaal" of "overal"), markeer ze dan als
          "geen koppeling nodig" — die telt dan niet meer mee als "nog te
          koppelen".
        </p>
...
function LocationCard({ groep, gekoppeld, genegeerd, onChanged }) {
  ...
  const negeer = async () => {
    setNegeerBezig(true);
    try {
      await LocationFactory.markeerGenegeerd(groep.label);
      onChanged();
    } finally {
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ fontFamily: fonts.body, fontSize: 12, color: colors.inkMuted }}>
          ...
          {genegeerd && (
            <span style={{ color: colors.inkMuted, fontWeight: 600 }}> · geen koppeling nodig</span>
          )}
          {!genegeerd && gekoppeld && (
      ...
      {genegeerd ? (
        <div style={{ marginTop: 10 }}>
          <p style={{ fontFamily: fonts.body, fontSize: 12, color: colors.inkMuted, margin: '0 0 8px' }}>
            Deze naam is gemarkeerd als "geen koppeling nodig" (bv. omdat het
            geen echte plaatsnaam is) en telt niet meer mee als "nog te
            koppelen".
          </p>
